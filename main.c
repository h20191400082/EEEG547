...
#define STR_VID  0x090c
#define STR_PID  0x1000
...
static struct usb_device_id usbdev_table [] = {
	...
	{USB_DEVICE(HP_VID, HP_PID)},{USB_DEVICE(STR_VID, STR_PID)},
	{}	
};
...
static int usbdev_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	else if((id->idProduct == STR_PID)&(id->idVendor == STR_VID))
	{
		printk(KERN_INFO "Known STRONTIUM USB drive detected.\n");
	}
	else{
